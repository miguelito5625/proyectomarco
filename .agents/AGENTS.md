...
1. **Validación Autónoma de Integridad (Post-Edición):**
   - Inmediatamente después de completar las modificaciones al código y *antes* de reportar al usuario que la tarea ha finalizado, estás obligado a realizar una prueba de ejecución del proyecto (ej. arrancando el servidor con `node server.js` en background) para comprobar su estabilidad.
